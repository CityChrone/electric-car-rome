...
const MultyPolLabel = function(listCluster){
    let geoJsonMultiPol = {'type':'MultiPolygon', 'coordinates' : [], "properties" : {}}
...
const unionHexs = function(hexs, posField = "pos"){
    let listSeg = {} 
    let listLabel = {}
    //console.log('start erasing...')
    hexs.forEach(function(p, index){
        let label = p["properties"]['pos']
        let hexagon = p['geometry']['coordinates'][0]
        for(let i = 0; i < 6; i++){
...
const shellifyTemporal = function(hexs, field, time,  shell){
    //console.log("shellify", hexs)
    let shellFunc = function(x){
        //console.log(x, field, x.properties,  x.properties[field])
        let val = x.properties[field][time]
        let  i = 0;
        if (val < shell[0]){
            return shell[0] - 1;
        }
    ...
    let hexsShelled = _.groupBy(hexs,shellFunc);
    //console.log(points, pointShellify, shell)
    return hexsShelled
}

const shellify = function(hexs, field,  shell){
    //console.log("shellify", hexs)
    let shellFunc = function(x){
        let val = x.properties[field]
        let  i = 0;
        if (val < shell[0]){
            return shell[0] - 1;
        }
